...
embedding_file_path_processed = os.path.join(os.path.dirname(__file__), 'data', 'yeast_ProtAlbert')
...
all_embeddings = get_file_with_cache("protein_ID_summarized.csv").copy()
embedding_UMAP = get_file_with_cache("protein_ID_summarized_UMAP.csv").copy()
proportions = get_file_with_cache("protein_ID_summarized_proportions.csv").copy()


st.sidebar.write("""### Polyprotein stats web app by Leon French
Embeddings for the human proteins are from the ProtT5 embedder at full precision (prottrans_t5_xl_u50 model) that were 
