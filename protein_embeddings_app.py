...
def get_split_embeddings():
    dfA = get_file_with_cache("ESM2_650M_embeddings_part_1.csv.gz")
    dfB = get_file_with_cache("ESM2_650M_embeddings_part_2.csv.gz")
    dfC = get_file_with_cache("ESM2_650M_embeddings_part_3.csv.gz")
...
embedding_UMAP = get_file_with_cache("gene_symbol_summarized_UMAP_ESM2_650M.csv").copy()
proportions = get_file_with_cache("gene_symbol_summarized_proportions.csv").copy()


st.sidebar.write("""### ESM2 650M protein embedding probe tool by Leon French
ESM2 650M embeddings for the human genes were generated by Jin Su. Full details on ESM2 by Lin et al. is available at [Science](https://www.science.org/doi/abs/10.1126/science.ade2574) and [Github](https://github.com/facebookresearch/esm) repository. The default gene list is from the [Lindbohm et al.](https://alz-journals.onlinelibrary.wiley.com/doi/10.1002/alz.12419) study of cognitive decline and risk of dementia.
...
* can the ESM2 640M embeddings, without finetuning discriminate the input genes from the rest of the proteome?
