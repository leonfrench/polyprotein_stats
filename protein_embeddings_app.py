...
embedding_file_path_processed = os.path.join(os.path.dirname(__file__), 'data', 'CZI_image_spectra')
...
all_embeddings = get_file_with_cache("feature_spectra.csv").copy()
...
st.sidebar.write("""### Polyprotein stats web app #2 by Leon French
The feature spectra used here are from the cytoself approach that was applied to 1,311 labeled proteins by Hirofumi Kobayashi, Keith Cheveralls, Manuel Leonetti and Loic Royer ([Self-supervised deep learning encodes high-resolution features of protein subcellular localization, Nature Methods](https://www.nature.com/articles/s41592-022-01541-z)).
...
sequence_input = "RPL10A\nRPL11\nRPL13\nRPL19\nRPL27\nRPL35\nRPL4\nRPL5\nRPS11\nRPS14\nRPS16\nRPS18\nRPL10A\n"

target_genes = st.sidebar.text_area("Target genes input (gene symbol per line)", sequence_input, height=100)

background_genes = st.sidebar.text_area("Background genes - leave blank for all proteins in dataset", "", height=100)
...
* can amino acid composition, plus sequence length discriminate the input proteins from the rest of the assayed proteins?
* can the image derived feature spectra discriminate the input proteins from the rest of the assayed proteins?

Source code is on [github](https://github.com/leonfrench/polyprotein_stats/tree/CZI_image_spectra)
""")
...
""")
    ...
    st.markdown(f"Using the learned feature spectra (see sidepanel for details), the average AUC is **{measures['AUC']:.2f}**.")
    ...
    get_download_button(X, y, all_embeddings, "CZI feature spectra")
...
The below plot shows the UMAP visualization of the feature spectra of the input (red, target=true on hover) and background proteins (grey). The blue dots mark top predicted 
proteins for each fold of the classification model (also top_predicted_hits_in_folds in the above dictionary). These predicted genes may help understand what the classifier is learning from the input proteins. The original spectra are in a sparse 2,048 dimensional space. 
