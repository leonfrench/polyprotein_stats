...
def get_split_embeddings():
    dfA = get_file_with_cache("subcell_all_lines_gene_symbol.csv.gz")
    return dfA
...
embedding_UMAP = get_file_with_cache("subcell_all_lines_gene_symbol.UMAP.csv").copy()
proportions = get_file_with_cache("gene_symbol_summarized_proportions.csv").copy()


st.sidebar.write("""### SubCell embedding probe tool by Leon French

The SubCell fluorescence microscopy based embeddings the human proteins was generated by Ankit Gupta and colleagues. Details about their models is available in the [preprint](https://www.biorxiv.org/content/10.1101/2024.12.06.627299v1.full). 

Embeddings were averaged across all data from all cell types and guided by the [embedding notebook](https://github.com/CellProfiling/subcell-analysis/blob/main/HPA_U2OS_embedding_subclustering_and_ORA.ipynb) by Konstantin Kahnert. Some of the UMAP clusters are probably due to proteins assayed in different combinations of cell types.
