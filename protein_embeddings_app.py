...
def get_split_embeddings():
    return get_file_with_cache("Nicheformer_genes_human_gene_symbol_summary.csv.gz")
...
embedding_UMAP = get_file_with_cache("gene_symbol_summarized_UMAP_nicheformer.csv").copy()
proportions = get_file_with_cache("gene_symbol_summarized_proportions.csv").copy()


st.sidebar.write("""### Nicheformer gene embedding probe tool by Leon French
Nicheformer (trained on SpatialCorpus-110M) based embeddings for the human genes were generated by Schaar, A.C., Tejada-Lapuerta, A., and colleagues (mouse data was used in training). Full details on Nicheformer are available at [bioRxiv](https://www.biorxiv.org/content/10.1101/2024.04.15.589472v1) and [Github](https://github.com/theislab/nicheformer/tree/main) repository. The default gene list is from the [Lindbohm et al.](https://alz-journals.onlinelibrary.wiley.com/doi/10.1002/alz.12419) study of cognitive decline and risk of dementia.
