...
def get_split_embeddings():
    return get_file_with_cache("all.500M_multi_species_v2.12k_length.embeddings.upstream.csv.gz")
...
embedding_UMAP = get_file_with_cache("gene_symbol_summarized_UMAP_nucleotide_trans.csv").copy()
proportions = get_file_with_cache("gene_symbol_summarized_proportions.csv").copy()


st.sidebar.write("""### Nucleotide transformer gene embedding probe tool by Leon French
Nucleotide transformer (500M_multi_species_v2) based embeddings for the human genes were generated by the Instadeepai model. Input was 12k nucleotides upstream of the TSS for canonical transcripts. Don't expect great accuracy, for reference it averages 0.56 AUROC as across GO groups. Full details on the Nucleotide transformer are available at [bioRxiv](https://www.biorxiv.org/content/10.1101/2023.01.11.523679v3) and [Github](https://github.com/instadeepai/nucleotide-transformer) repository. The default gene list is from the [Lindbohm et al.](https://alz-journals.onlinelibrary.wiley.com/doi/10.1002/alz.12419) study of cognitive decline and risk of dementia.
